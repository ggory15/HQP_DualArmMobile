...
void EiquadprogFast::reset(Indexnum nVars,
	Indexnum nEqCon,
	Indexnum nIneqCon)
{
	m_nVars = nVars;
...
EiquadprogFast_status EiquadprogFast::solve_quadprog(const MatrixXd & Hess,
	...
	VectorXd & x)
{
	const Indexnum nVars = g0.size();
	const Indexnum nEqCon = ce0.size();
	const Indexnum nIneqCon = ci0.size();

	if (nVars != m_nVars || nEqCon != m_nEqCon || nIneqCon != m_nIneqCon)
		reset(nVars, nEqCon, nIneqCon);

	assert(static_cast<Indexnum>(Hess.rows()) == m_nVars && static_cast<Indexnum>(Hess.cols()) == m_nVars);
	assert(static_cast<Indexnum>(g0.size()) == m_nVars);
	assert(static_cast<Indexnum>(CE.rows()) == m_nEqCon && static_cast<Indexnum>(CE.cols()) == m_nVars);
	assert(static_cast<Indexnum>(ce0.size()) == m_nEqCon);
	assert(static_cast<Indexnum>(CI.rows()) == m_nIneqCon && static_cast<Indexnum>(CI.cols()) == m_nVars);
	assert(static_cast<Indexnum>(ci0.size()) == m_nIneqCon);

	int i, k, l;  // indices

	Indexnum ip;       // index of the chosen violated constraint
	Indexnum iq;       // current number of active constraints
