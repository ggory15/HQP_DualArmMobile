...
namespace HQP {
	...
	template <typename D>
	Eigen::Matrix<typename D::Scalar, 3, 3, D::Options>
	...
	VectorXd log3(MatrixXd & R, double & theta) {
	...
	/*Transform3d inverse(const Transform3d & T) {
		Transform3d res;
		res.linear() = T.linear().transpose();
		res.translation() = -1.0*res.linear()*T.translation();
		return res;
	}*/
}
